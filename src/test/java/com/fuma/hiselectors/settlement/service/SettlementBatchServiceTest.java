...
class SettlementBatchServiceTest {
    ...
    @Test
    void finalizesFromTheTwentySecondAndContinuesAfterIndividualFailure() {
        ...
        SettlementBatchService service =
                new SettlementBatchService(selectorsRepository, worker, clock);
        ...
        when(worker.calculate(
                ...
                eq(SettlementSourceCode.DAILY_BATCH), eq(true)))
        ...
        SettlementBatchService.SettlementBatchResult result =
                service.calculatePreviousMonth();

        assertThat(result.finalized()).isTrue();
        assertThat(result.processedCount()).isEqualTo(1);
        assertThat(result.failedCount()).isEqualTo(1);
        verify(worker).calculate(
                2L, YearMonth.of(2026, 7), SettlementSourceCode.DAILY_BATCH, true);
    }
}
