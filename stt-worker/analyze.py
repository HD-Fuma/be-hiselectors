...
HATE_MODEL = "smilegate-ai/kor_unsmile"
MIN_CONFIDENCE = 0.3
HATE_THRESHOLD = 0.5
CHUNK_CHARS = 300
_STOP = frozenset({
...
def _selfcheck() -> None:
    ...
    assert "hate" not in beauty, "기본은 욕설혐오 안 돌려야 함"

    opt = analyze("정상 텍스트", include_hate=True)
    assert opt["hate"]["suspected"] is False, "opt-in hate 실패"
