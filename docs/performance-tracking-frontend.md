...
응답 `SelectorMatchResponse[]` (추천 점수 순):
```jsonc
[
  {
    "selectorId": 12,
    ...
    "conversionRate": 12.5,        // 전환율 %(카테고리 상품 클릭 대비 확정주문), 클릭 없으면 null
    "matchScore": 87,              // 추천 점수 0~100 (후보 내 상대 점수)
    "representativeMatch": true,    // 대표 카테고리 일치 여부
    "fallback": false,             // 실적 없이 대표카테고리만으로 보완된 항목인지
    "matchReason": "확정매출 3,400,000원 · 주문 42건 · 전환율 12.5%"
  }
]
```

**추천 로직(개선됨 — 단순 매출순 아님)**
- **실적자 우선**: 해당 카테고리에서 확정매출이 있는 셀렉터스를 먼저 추천.
- **점수 = 최근성 가중 매출(0.6) + 전환율(0.4)** 를 후보 집단 내에서 정규화해 합산.
  - *최근성*: 최근 90일 확정매출에 더 큰 가중치(오래된 매출은 0.4배).
  - *전환율*: 카테고리 상품 클릭 대비 확정주문. 적게 노출되고도 잘 파는 셀렉터스가 상위로.
  - → 오래·많이 팔아온 "매출왕"만 반복 추천되는 편향을 완화.
- **보완(fallback)**: 실적자가 `limit`에 못 미칠 때만, 대표 카테고리가 일치하는(실적 0) 셀렉터스로 채움. 이 항목은 `fallback: true`, `categorySales: 0`, `matchScore: 0`, `conversionRate: null`. FE에서 "실적 없음 · 대표 카테고리 일치"로 구분 표시 권장.
- 삭제·블랙리스트 셀렉터스는 제외. 본인 구매는 매출 집계에서 제외.
